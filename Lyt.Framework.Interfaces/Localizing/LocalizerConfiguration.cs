...
public sealed class LocalizerConfiguration
{
    ...
    public string LanguagesFileExtension { get; set; } = ".lang"; // includes dot
    ...
    public string ResourcePathString()
        => string.Format("{0}/{1}", this.AssemblyName, this.LanguagesSubFolder);

    public string ResourceFileEmbeddedPathString(string targetLanguage)
        => string.Format(
            "{0}{1}{2}", this.LanguagesFilePrefix, targetLanguage, this.LanguagesFileExtension);
    ...
    public string ResourceFolderUriString()
        => string.Format("avares://{0}/{1}", this.AssemblyName, this.LanguagesSubFolder);
}
